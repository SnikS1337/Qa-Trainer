      ...
      <div className="max-w-[600px] mx-auto w-full p-6 flex flex-col flex-1">
        ...
        <div className="flex-1 flex flex-col gap-3">
          {(q.opts || []).map((opt: string, idx: number) => (
            <button
              ...
              className={`text-left p-4 rounded-2xl border-[1.5px] transition-all duration-220 backdrop-blur-md ${selectedOption === idx ? 'border-brand-purple/50 bg-brand-purple/10 text-white' : 'border-white/10 bg-white/5 text-slate-300 hover:bg-white/8 hover:border-white/24 hover:shadow-[0_0_8px_rgba(255,255,255,0.06)]'}`}
            >
        ...
        <div className="mt-8">
          <button 
            ...
            className={`w-full font-bold py-4 rounded-xl uppercase tracking-wide transition-all duration-220 backdrop-blur-md border ${selectedOption !== null ? 'bg-brand-purple/80 border-brand-purple/50 text-white hover:bg-brand-purple hover:shadow-[0_0_12px_rgba(167,139,250,0.22)]' : 'bg-black/20 border-white/5 text-slate-500 cursor-not-allowed'}`}
          >
